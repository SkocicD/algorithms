...
def addtotree(currtree, cheese):
    # currtree[cheese] = {"maxdepth": 1}
    currtree[cheese] = {}
    for treecheese in currtree:
        # if treecheese == "maxdepth":
        #     continue
        if isInside(cheese, treecheese):
            addtotree(currtree[treecheese], cheese)
            # if currtree[treecheese]["maxdepth"] > currtree["maxdepth"]:
            #     currtree["maxdepth"] = currtree[treecheese]["maxdepth"]
